...
function AmountInput({ amountInput, setAmountInput, setAmount, isListening, setIsListening, currencies, setFromCurrency, setToCurrency }) {
    const handleAmountChange = (e) => {
        ...
        try {
            ...
            toast.error('Invalid expression! Use numbers and operators like +, -, *, /');
            setAmount('');
        }
    };
    ...
    const startVoiceInput = () => {
        ...
        recognition.onresult = (event) => {
            ...
            const fromCurrency = words.slice(0, toIndex).find(word => /^[A-Z]{3}$/.test(word.toUpperCase()));
            const toCurrency = words.slice(toIndex + 1).find(word => /^[A-Z]{3}$/.test(word.toUpperCase()));
            ...
            if (amountValue && fromCurrency && toCurrency &&
                ...
                toast.error('Could not understand voice input. Say, e.g., "Convert 100 USD to PKR"');
            }
        };
        ...
        <div className='mb-8'>
            ...
            <div className='flex gap-3'>
                ...
                <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
                    ...
                    >
                        {isListening ? <LuMic className='h-5 w-5' /> : <FaMicrophoneSlash className='h-5 w-5' />}
                    </Button>
                </motion.div>
            </div>
        </div>
